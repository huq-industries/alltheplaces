...
class RadissonHotelsSpider(scrapy.Spider):
    ...
    brand_mapping = {'rdb': ['Radisson Blu', 'Q7281341'], 'rco': ['Radisson Collection', 'Q60716706'], 'pii': ['Park Inn by Radisson', 'Q60711675'], 'rdr': ['Radisson RED', 'Q28233721'], 'cis': ['Country Inn & Suites by Radisson', 'Q5177332'], 'pph': ['Park Plaza Hotels & Resorts', 'Q2052550'], 'art': ['Art’otel', 'Q14516231'], 'rad': ['Radisson', 'Q1751979'], 'prz': ['Prizeotel', None], 'rdi': ['Radisson Individuals', None], 'ri': ['Radisson Individuals', None]}
    custom_settings = {'DEFAULT_REQUEST_HEADERS': {'Accept': '*/*', 'User-Agent': BROWSER_DEFAULT, 'Connection': 'keep-alive', 'Accept-Language': 'fr-FR'}}
    ...
    def parse_city(self, response):
        try:
            hotels: List[dict] = json.loads('[' + re.search('({"code":.*{.*})', response.text).group(1) + ']')
