...
class FlemingsSteakhouseSpider(scrapy.Spider):
    ...
    item_attributes = {'brand': "Fleming's Prime Steakhouse & Wine Bar", 'brand_wikidata': 'Q5458552'}
    ...
    def parse_location(self, response):
        ...
        city, state, postcode = re.search('^(.*), ([A-Z]{2}) (\\d{5})$', city_state_postal_element).groups()
        properties = {'ref': re.search('.+/(.+?)/?(?:\\.html|$)', response.url).group(1), 'name': response.xpath('//div[@id="location-details"]/h1/text()').extract_first(), 'addr_full': address, 'city': city, 'state': state, 'postcode': postcode, 'website': response.url, 'phone': response.xpath('//a[@class="phone"]/text()').extract_first()}
