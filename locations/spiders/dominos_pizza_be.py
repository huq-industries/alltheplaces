...
class DominosPizzaBESpider(SitemapSpider):
    ...
    url_regex = 'https:\\/\\/www\\.dominos\\.be/nl/winkel\\/([\\w]+)-([\\w]+)-([\\d]+)$'
    ...
    def parse_store(self, response):
        ...
        locality_data = re.search('(.*) ([A-Z]{2}) (.*)', address_data[1].strip())
        properties = {'ref': ref.strip('/'), 'name': response.xpath('//h1[@class="storetitle"]/text()').extract_first(), 'street_address': address_data[0].strip().strip(','), 'country': country, 'lat': response.xpath('//input[@id="store-lat"]/@value').get().replace(',', '.'), 'lon': response.xpath('//input[@id="store-lon"]/@value').get().replace(',', '.'), 'website': response.url}
        if locality_data:
