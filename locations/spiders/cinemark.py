...
class CinemarkSpider(SitemapSpider):
    ...
    sitemap_rules = [('https:\\/\\/www\\.cinemark\\.com\\/theatres\\/', 'parse')]

    def parse(self, response):
        item = LinkedDataParser.parse(response, 'MovieTheater')
        item['ref'] = '/'.join(response.url.rsplit('/')[-2:])
        item['lat'], item['lon'] = parse_qs(urlparse(response.css('.theatreInfoCollapseMap').xpath('//a/img/@data-src').extract_first()).query)['pp'][0].split(',')
        yield item
    new_property = None