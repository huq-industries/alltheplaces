...
class DickBlickSpider(SitemapSpider, StructuredDataSpider):
    ...
    sitemap_rules = [('/stores/[-\\w]+/[-\\w]+/$', 'parse_sd')]
    ...
    def inspect_item(self, item, response):
        ...
        for day in response.xpath('//li[contains(@data-testid,"hours")]'):
            if day.xpath('./text()').get().strip() == 'Closed':
                continue
            oh.add_range(day=day.xpath('normalize-space(./span/text())').get().strip(':')[:3], open_time=day.xpath('./text()').get().split(' - ')[0], close_time=day.xpath('./text()').get().split(' - ')[1], time_format='%I:%M%p')
