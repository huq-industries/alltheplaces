...
class FreshThymeSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for location in data:
            ...
            properties = {'name': location['name'], 'ref': location['id'], 'street': ', '.join(filter(None, [address.get(f'address{x}') for x in range(1, 4)])), 'city': address['city'], 'postcode': address['postal_code'], 'state': address['province'], 'country': 'US', 'phone': location['phone_number'], 'website': location['external_url'], 'lat': float(coordinates['latitude']), 'lon': float(coordinates['longitude']), 'opening_hours': self.parse_hours(hours) if hours else None}
