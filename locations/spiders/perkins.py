...
class PerkinsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for path in city_urls:
            if path.count('/') == 6:
                yield scrapy.Request(path, callback=self.parse_store)

    def parse_store(self, response):
        properties = {'addr_full': response.xpath('//span[@itemprop="streetAddress"]/span/text()').extract_first(), 'city': response.xpath('//span[@itemprop="addressLocality"]/text()').extract_first(), 'state': response.xpath('//abbr[@itemprop="addressRegion"]/text()').extract_first(), 'postcode': response.xpath('//span[@itemprop="postalCode"]/text()').extract_first().strip(), 'ref': response.url, 'website': response.url, 'lon': float(response.xpath('//span/meta[@itemprop="longitude"]/@content').extract_first()), 'lat': float(response.xpath('//span/meta[@itemprop="latitude"]/@content').extract_first())}
