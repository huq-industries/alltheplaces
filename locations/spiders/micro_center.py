...
class MicroCenterSpider(scrapy.Spider):
    ...
    def parse_store(self, response):
        ...
        properties = {'ref': response.url, 'name': data['name'], 'website': response.url, 'phone': data['telephone'], 'street_address': data['address']['streetAddress'], 'city': data['address']['addressLocality'], 'state': data['address']['addressRegion'], 'postcode': data['address']['postalCode'], 'country': data['address']['addressCountry'], 'lat': data['geo']['latitude'], 'lon': data['geo']['longitude'], 'opening_hours': opening_hours.as_opening_hours()}
