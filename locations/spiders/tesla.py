...
class TeslaSpider(scrapy.Spider):
    ...
    def start_requests(self):
        yield scrapy.Request('https://www.tesla.com/cua-api/tesla-locations?translate=en_US&usetrt=true', callback=self.parse_json_subrequest)
    ...
    def parse_json_subrequest(self, response):
        for location in response.json():
            ...
            if location.get('location_type') == ['destination charger']:
                continue
            yield scrapy.Request(url=f"https://www.tesla.com/cua-api/tesla-location?translate=en_US&usetrt=true&id={location.get('location_id')}", callback=self.parse_location)
    ...
    def parse_location(self, response):
        ...
        if 'supercharger' in location_data.get('location_type'):
            ...
            regex = '(\\d+) Superchargers, available 24\\/7, up to (\\d+kW)(<br />CCS Compatibility)?'
