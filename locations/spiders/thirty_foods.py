...
class ThirtyFoodsSpider(scrapy.Spider):
    ...
    start_urls = ('https://www.thriftyfoods.com/api/en/Store/get?Latitude=48.45423&Longitude=-123.359205&Skip=0&Max=60000',)
    ...
    def store_hours(self, store_hours):
        ...
        if 'Open 24 hours' in store_hours:
            ...
            hours = ''
            match = re.search('(\\d{1,2}):(\\d{2}) (A|P)M - (\\d{1,2}):(\\d{2}) (A|P)M', store_hours)
            if match:
    ...
    def parse(self, response):
        ...
        for store in results['Data']:
            properties = {'ref': store['Number'], 'name': store['Name'], 'lat': store['Coordinates']['Latitude'], 'lon': store['Coordinates']['Longitude'], 'addr_full': store['AddressMain']['Line'], 'city': store['AddressMain']['City'], 'state': store['AddressMain']['Province'], 'postcode': store['AddressMain']['PostalCode'], 'phone': store['PhoneNumberHome']['Number'], 'opening_hours': self.store_hours(store['OpeningHours'])}
