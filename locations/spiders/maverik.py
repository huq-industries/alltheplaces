...
class MaverikSpider(scrapy.Spider):
    ...
    def add_fuels(self, response):
        yield scrapy.Request('https://gateway.maverik.com/ac-loc/location/fuel/all', headers={'APP-ID': 'PAYX'}, meta=response.json())
    ...
    def parse(self, response):
        ...
        for location in locations:
            ...
            if not ('longitude' in location and 'latitude' in location):
                continue
            item = Feature(lon=location['longitude'], lat=location['latitude'], ref=location['code'], extras={'branch': location['name']}, street_address=address['address1'], city=address['city'], state=address['stateProvince'], postcode=address['postalCode'], country=address['country'], phone=address['phone'], opening_hours='24/7' if '12:00AM - 12:00AM' == location.get('hoursOfOperation') else None)
