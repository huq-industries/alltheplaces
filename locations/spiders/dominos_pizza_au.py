...
class DominosPizzaAUSpider(scrapy.Spider):
    ...
    def pares_store(self, response):
        ...
        country = re.search('com\\.([a-z]{2})\\/', response.url).group(1)
        properties = {'ref': ref.strip('/'), 'name': response.xpath('//div[@class="storetitle"]/text()').extract_first(), 'addr_full': clean_address(response.xpath('//a[@id="open-map-address"]/text()').getall()), 'country': country, 'lat': float(response.xpath('//div[@class="store-details-info"]/div[1]/input[1]/@value').extract_first()), 'lon': float(response.xpath('//div[@class="store-details-info"]/div[1]/input[2]/@value').extract_first()), 'website': response.url}
