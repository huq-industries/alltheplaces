...
class FollettSpider(Spider):
    ...
    def parse(self, response):
        ...
        if location['state'] != 'open':
            return
        properties = {'ref': location['storeId'], 'phone': location['storeExtensionData'][0].get('phone1'), 'twitter': clean_twitter(location['storeExtensionData'][0].get('twitterUrl', '')), 'facebook': clean_facebook(location['storeExtensionData'][0].get('facebookUrl', '')), 'website': 'https://www.bkstr.com/' + response.url.split('storeName=', 1)[1] + '/home'}
        for name_data in location['description']:
            if name_data['languageId'] == '-1':
                properties['name'] = name_data['displayName']
                break
        for location_data in location['locationInfo']:
            if location_data['languageId'] == '-1':
