...
class AlbertHeijnSpider(Spider):
    ...
    def get_page(self, gql_url, page_number):
        ...
        query = [{'operationName': 'stores', 'query': gql_query, 'variables': {'filter': {}, 'size': 100, 'start': page_number * 100}}]
        headers = {'client-name': 'alltheplaces', 'client-version': '1', 'Origin': gql_url.replace('/gql', ''), 'Referer': gql_url.replace('/gql', '/winkels')}
