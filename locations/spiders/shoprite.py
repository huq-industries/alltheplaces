...
kTimesRe = re.compile('(\\d+)(:\\d+)? *([APap][Mm])')
...
class ShopriteSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for store in stores:
            ...
            properties = {'ref': ref, 'website': f'https://www.shoprite.com/sm/planning/rsid/{ref}', 'name': store['name'], 'lat': store['location']['latitude'], 'lon': store['location']['longitude'], 'street_address': store['addressLine1'], 'city': store['city'], 'state': store['countyProvinceState'], 'postcode': store['postCode'], 'country': store['country'], 'phone': store['phone'], 'opening_hours': self.parse_hours(store['openingHours'])}
