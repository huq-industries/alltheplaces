...
class VeaSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for store in store_list:
            properties = {'name': store['descripcion'], 'addr_full': store['direccion'], 'city': store['vea_localidades_desc'], 'opening_hours': store['horarios'], 'phone': store['telefonos'], 'ref': str(store['codigo']).replace('SM ', '')}
