...
class ArgosSpider(SitemapSpider):
    ...
    sitemap_rules = [('https://www.argos.co.uk/stores/([\\d]+)-([\\w-]+)', 'parse')]
    user_agent = BROWSER_DEFAULT

    def parse(self, response):
        data = re.findall('window.INITIAL_STATE =[^<]+', response.text)
        json_data = json.loads(data[0].replace('window.INITIAL_STATE =', ''))
        properties = {'street_address': json_data['store']['store']['address'], 'phone': json_data['store']['store']['tel'], 'city': json_data['store']['store']['town'], 'name': json_data['store']['store']['name'], 'postcode': json_data['store']['store']['postcode'], 'ref': json_data['store']['store']['id'], 'website': response.url, 'lat': float(json_data['store']['store']['lat']), 'lon': float(json_data['store']['store']['lng']), 'extras': {'store_type': json_data['store']['store']['type']}}
        oh = OpeningHours()
