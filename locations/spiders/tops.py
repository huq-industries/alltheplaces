...
day_formats = {'Mon': 'Mo', 'Tues': 'Tu', 'Wed': 'We', 'Thur': 'Th', 'Fri': 'Fr', 'Sat': 'Sa', 'Sun': 'Su'}
...
class TopsSpider(scrapy.Spider):
    ...
    def store_hours(self, store_hours):
        ...
        for day, hours_together in m:
            ...
            if to_ap == 'PM' and to_h != 12:
                to_h += 12
            hours = '{:02}:{}-{:02}:{}'.format(from_h, from_m, to_h, to_m)
    ...
    def parse_store(self, response):
        ...
        properties = {'addr_full': address, 'city': city, 'state': state, 'postcode': postalCode, 'phone': phone, 'ref': response.meta['ref'], 'lon': response.meta['lon'], 'lat': response.meta['lat']}
