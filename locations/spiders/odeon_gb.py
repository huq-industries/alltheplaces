...
class OdeonGBSpider(Spider):
    ...
    def parse(self, response, **kwargs):
        ...
        for location in data['config']['cinemas']:
            location['address'] = ', '.join(filter(None, [location.pop('addressLine1'), location.pop('addressLine2'), location.pop('addressLine3'), location.pop('addressLine4'), 'United Kingdom']))
