...
class PrimroseSchoolsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        with open_searchable_points('us_centroids_50mile_radius.csv') as points:
            ...
            for point in points:
                ...
                lati = row[1]
                long = row[2]
                searchurl = 'https://www.primroseschools.com/find-a-school/?search_string=USA&latitude={la}&longitude={lo}'.format(la=lati, lo=long)
    ...
    def parse_search(self, response):
        ...
        for i in schools:
            if i['address_1']:
                properties = {'name': i['name'], 'street_address': ', '.join(filter(None, [i['address_1'], i['address_2']])), 'city': i['city'], 'state': i['state'], 'postcode': i['zip_code'], 'phone': i['phone'], 'ref': i['id'], 'website': 'https://www.primroseschools.com' + i['url'], 'lat': float(i['latitude']), 'lon': float(i['longitude'])}
