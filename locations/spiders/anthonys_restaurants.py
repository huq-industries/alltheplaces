...
class AnthonysRestaurantsSpider(scrapy.Spider):
    ...
    def parse_location(self, response):
        ...
        properties = {'ref': re.search('postid-(\\d+)', response.css('body').attrib['class'])[1], 'lat': address['latitude'], 'lon': address['longitude'], 'street_address': address['address'], 'city': address['city'], 'state': address['state'], 'postcode': address['zip_code'], 'name': name, 'website': response.url, 'phone': (response.xpath("//*[starts-with(@href, 'tel:')]/@href").get() or '')[4:], 'opening_hours': hours}
