...
class HEBUSSpider(Spider):
    ...
    def start_requests(self):
        graphql_query = [{'extensions': {'persistedQuery': {'sha256Hash': '24dc544e4b33ce6995dff6650e4deb9ba53e8c4a928b33535a710b96bd2517d6', 'version': 1}}, 'operationName': 'StoreDetailsSearch', 'variables': {'address': '90210', 'radius': 10000, 'size': 5000}}]
    ...
    def parse(self, response):
        for location in response.json()[0]['data']['searchStoresByAddress']['stores']:
            ...
            item['website'] = 'https://www.heb.com/heb-store/' + item['country'] + '/' + item['state'].lower() + '/' + item['city'].lower() + '/' + re.sub('[^\\w]', '-', item['name'].lower()) + '-' + str(item['ref'])
