        ...
        for i in results:
            if i['locationType'] == 'STORE':
                if i['storeBannerId'] == 'dominion':
                    ...
                    wikidata = 'Q8058833'
                properties = {'brand': brand, 'brand_wikidata': wikidata, 'ref': i['storeId'], 'name': i['name'], 'lat': i['geoPoint']['latitude'], 'lon': i['geoPoint']['longitude'], 'street_address': ', '.join(filter(None, [i['address'].get('line2'), i['address'].get('line1')])), 'city': i['address']['town'], 'state': i['address']['region'], 'postcode': i['address']['postalCode'], 'country': i['address']['country']}
