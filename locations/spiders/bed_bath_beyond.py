...
class BedBathBeyondSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for url in data:
            yield scrapy.Request(f"https://www.{self.allowed_domains[0]}/locations/state/{url['code']}", callback=self.parse_store)
        ...
        for store in stores:
            ...
            for timing in storeTimings:
                ...
                for day in re.split('-| - ', timing.split(': ')[0]):
                    oh.add_range(day=day, open_time=re.split('-| - ', timing.split(': ')[1])[0], close_time=re.split('-| - ', timing.split(': ')[1])[1], time_format='%I:%M%p')
