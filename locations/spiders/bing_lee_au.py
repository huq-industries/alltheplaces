...
class BingLeeAUSpider(SitemapSpider, StructuredDataSpider):
    ...
    def post_process_item(self, item, response, ld_data, **kwargs):
        item['street_address'] = ' '.join(item['street_address'].split()).replace('Click &amp; Collect Hub Only - Please note: this is not a storefront., ', '').replace('&amp;', '&').replace('O&#x27;', '').replace(',,', ',').replace(' ,', ',')
        item.pop('facebook')
