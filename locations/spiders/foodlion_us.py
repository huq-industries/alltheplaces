    ...
    def parse_hours(hours: [str]) -> OpeningHours:
        ...
        for rule in hours:
            if rule == 'Open 24 Hours':
                return '24/7'
            if (m := re.match('(\\w+)(?:-(\\w+))?: (\\d+:\\d\\d)\\s*([ap]m)\\s*-\\s*(\\d+:\\d\\d)\\s*([ap]m)', rule.replace('.', ''))):
                start_day, end_day, start_time, start_zone, end_time, end_zone = m.groups()
                if not end_day:
                    end_day = start_day
                oh.add_days_range(day_range(start_day, end_day), f'{start_time} {start_zone}', f'{end_time} {end_zone}', time_format='%I:%M %p')
        return oh
