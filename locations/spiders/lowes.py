...
class LowesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        yield scrapy.Request(url='https://www.lowes.com/sitemap/store0.xml', callback=self.parse)
    ...
    def parse_hours(self, store_hours):
        ...
        for weekday in store_hours:
            ...
            opening_hours.add_range(day=day_mapping[day], open_time=open_time_formatted, close_time=close_time_formatted)
    ...
    def parse_store(self, response):
        ...
        properties = {'lat': json_data['storeDetails']['lat'], 'lon': json_data['storeDetails']['long'], 'ref': json_data['storeDetails']['id'], 'addr_full': json_data['storeDetails']['address'], 'city': json_data['storeDetails']['city'], 'state': json_data['storeDetails']['state'], 'postcode': json_data['storeDetails']['zip'], 'phone': json_data['storeDetails']['phone'], 'website': response.request.url, 'opening_hours': self.parse_hours(store_hours), 'extras': {'amenity:toilets': True}}
