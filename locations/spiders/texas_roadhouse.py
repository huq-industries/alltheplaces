...
class TexasRoadhouseSpider(scrapy.Spider):
    ...
    def parse_store(self, response):
        ...
        properties = {'lat': data['latitude'], 'lon': data['longitude'], 'ref': data['url'], 'name': data['name'], 'addr_full': data['address1'], 'city': data['city'], 'state': data['state'], 'postcode': data['postalCode'], 'country': data['countryCode'], 'phone': data['telephone'], 'website': response.urljoin(data['url']), 'opening_hours': self.parse_hours(data['schedule'])}
