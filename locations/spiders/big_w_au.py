...
class BigWAUSpider(Spider):
    ...
    def parse(self, response):
        ...
        for store in stores:
            properties = {'ref': store['id'], 'name': store['n'], 'phone': store['ph'], 'street_address': store['a']['street'], 'city': store['a']['suburb'], 'state': store['a']['state'], 'postcode': store['a']['postcode'], 'lat': store['l']['lat'], 'lon': store['l']['lng']}
