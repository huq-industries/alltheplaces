...
class TerryWhiteChemmartAUSpider(scrapy.Spider):
    ...
    item_attributes = {'brand': 'Terry White Chemmart', 'brand_wikidata': 'Q24089773', 'country': 'AU'}
    allowed_domains = ['terrywhitechemmart.com.au']

    def start_requests(self):
        yield JsonRequest(url='https://terrywhitechemmart.com.au/store-api/get-stores-summary', data={}, method='POST')
    ...
    def parse(self, response):
        ...
        for store in stores['data']:
            ...
            for day in DAYS:
                for start_time, end_time in re.findall(day + '(?:(\\d{4})(\\d{4})|X{8})', store['availability'], flags=re.IGNORECASE):
