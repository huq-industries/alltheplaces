...
class AthletaSpider(scrapy.Spider):
    ...
    def parse_store(self, response):
        ...
        properties = {'phone': json_data['local_phone'], 'ref': json_data['fid'], 'name': json_data['location_name'], 'opening_hours': oh.as_opening_hours(), 'street_address': json_data['address_1'], 'housenumber': json_data['address_1'].strip().split()[0], 'street': json_data['address_1'].replace(json_data['address_1'].strip().split()[0], '').strip(), 'city': json_data['city'], 'state': json_data['region'], 'postcode': json_data['post_code'], 'country': json_data['country'], 'website': self.athleta_store_url + '/' + json_data['region'].lower() + '/' + json_data['city'].lower() + '/' + json_data['url_search_term'], 'lat': json_data['lat'], 'lon': json_data['lng']}
