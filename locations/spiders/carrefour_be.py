...
class CarrefourBESpider(scrapy.Spider):
    ...
    brands = {'express': ('Carrefour Express', 'Q2940190', None), 'orange': ('Carrefour Express', 'Q2940190', None), 'market': ('Carrefour Market', 'Q2689639', Categories.SHOP_SUPERMARKET), 'hyper': ('Carrefour', 'Q217599', Categories.SHOP_SUPERMARKET), 'drive-2': ('Carrefour', 'Q217599', {**Categories.SHOP_SUPERMARKET.value, 'drive_through': 'yes'})}
    ...
    def parse(self, response):
        for data in response.json():
            ...
            item['website'] = 'https://winkels.carrefour.be/nl/s/carrefour/{slug}/{id}'.format(slug=data.get('slug'), id=data.get('externalId'))
            yield item
    requires_proxy = True
    requires_proxy = True
    requires_proxy = True