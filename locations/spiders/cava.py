...
class CavaSpider(scrapy.Spider):
    ...
    def parse_location(self, location):
        ...
        street_address = location.xpath('.//div[@class="street-address"]/text()').extract_first().replace(nonBreakingSpace, ' ') + ', ' + location.xpath('.//span[@class="locality"]/text()').extract_first().replace(nonBreakingSpace, ' ')
        ...
        properties = {'ref': street_address, 'street_address': street_address, 'city': city, 'postcode': postcode, 'state': state, 'phone': phone, 'opening_hours': opening_hours}
