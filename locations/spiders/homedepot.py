...
class HomeDepotSpider(CrawlSpider, StructuredDataSpider):
    ...
    rules = [Rule(LinkExtractor(allow='^https:\\/\\/www.homedepot.com\\/l\\/..$')), Rule(LinkExtractor(allow='^https:\\/\\/www.homedepot.com\\/l\\/.*\\/\\d*$'), callback='parse_sd')]
    requires_proxy = 'US'

    def post_process_item(self, item, response, ld_data, **kwargs):
        data = json.loads(response.xpath('//script[contains(text(), "__APOLLO_STATE__")]/text()').extract_first().strip()[24:-1])['ROOT_QUERY']
        store_info = None
