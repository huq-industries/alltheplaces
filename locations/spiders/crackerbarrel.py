...
class CrackerBarrelSpider(SitemapSpider):
    ...
    sitemap_rules = [('\\/Locations\\/States\\/(\\w{2})\\/([-\\w]+)\\/(\\d+)$', 'parse_store')]
    requires_proxy = True
    ...
    def parse_store(self, response):
        ...
        properties = {'ref': data['Store Number']['value'], 'lat': data['Latitude']['value'], 'lon': data['Longitude']['value'], 'website': response.url, 'name': data['Alternate Name']['value'], 'street_address': data['Address 1']['value'], 'city': data['City']['value'], 'state': data['State']['value'], 'postcode': data['Zip']['value'], 'country': data['Country']['value'], 'phone': data['Phone']['value'], 'opening_hours': hours.as_opening_hours()}
