...
class OttosCHSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for store in json.loads(js)['items']:
            props = {'lat': float(store['lat']), 'lon': float(store['lng']), 'name': 'Otto’s', 'city': store['city'], 'country': store['country'], 'opening_hours': opening_hours.get(store['name']), 'phone': store['phone'], 'postcode': store['zip'], 'ref': store['id'], 'street_address': store['address']}
            item = Feature(**props)
