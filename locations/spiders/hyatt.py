...
class HyattSpider(scrapy.Spider):
    ...
    regions = ('1-NorthAmerica', '2-CaribbeanAndLatinAmerica', '3-Europe', '4-AfricaAndMiddleEast', '5-Asia', '6-Australia')
    ...
    def parse_hotel(self, response):
        ...
        properties = {'ref': '_'.join(response.url.split('/')[-3:]), 'name': data['name'], 'addr_full': data['address']['streetAddress'], 'city': data['address']['addressLocality'], 'state': data['address']['addressRegion'], 'postcode': data['address']['postalCode'], 'country': data['address']['addressCountry'], 'phone': data.get('telephone', None), 'lat': float(data['geo']['latitude']), 'lon': float(data['geo']['longitude']), 'website': response.url}
