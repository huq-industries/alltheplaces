...
class ExtraSpaceStorageSpider(SitemapSpider):
    ...
    sitemap_rules = [('/facilities/', 'parse')]
    ...
    def parse(self, response):
        ...
        properties = {'ref': response.url.split('/')[-2], 'name': data['name'], 'street_address': data_address['streetAddress'], 'city': data_address['addressLocality'], 'state': data_address['addressRegion'], 'postcode': data_address['postalCode'], 'lon': float(data_geo['longitude']), 'lat': float(data_geo['latitude']), 'phone': data['telephone'], 'website': data['url'], 'opening_hours': opening_hours.as_opening_hours()}
