...
class PriceRiteUSSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for store in stores:
            store['street_address'] = clean_address([store['addressLine1'], store['addressLine2'], store['addressLine3']])
            ...
            for start_day, end_day, start_time, start_zone, end_time, end_zone in re.findall('(\\w+)(?:\\s*-\\s*(\\w+))?:? (\\d+)\\s*([ap]m)\\s*-\\s*(\\d+)\\s*([ap]m)', store['openingHours'].replace('\\n', ' ').replace(' to ', '-')):
                if not end_day:
                    end_day = start_day
