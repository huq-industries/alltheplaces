...
class MisterCarWashSpider(scrapy.Spider):
    ...
    def store_hours(self, store_hours):
        ...
        for line in store_hours.split(';'):
            match = re.search('\\s*(\\d{1,2}):(\\d{1,2})\\s*(am|pm|mp)?\\s*-\\s*(\\d{1,2}):(\\d{1,2})\\s*(am|pm|mp)\\s*(\\w*)(\\s*-\\s*(\\w*))?', line)
            stri += match[7][:2]
            try:
                stri += match[8][:3] + ' '
            except Exception:
    ...
    def phone_normalize(self, phone):
        r = re.search('\\+?(\\s+)*(\\d{1})?(\\s|\\()*(\\d{3})(\\s+|\\))*(\\d{3})(\\s+|-)?(\\d{2})(\\s+|-)?(\\d{2})', phone)
    ...
    def parse(self, response):
        ...
        for wash in wash_list:
            address_parts = re.match('(.*),\\s*(\\D{2,}\\s?\\D{2,}?\\s?\\D*)\\s*,\\s*(\\D{2})\\s*(\\d{5})?,(\\D{4,})?', wash['address'])
            if not address_parts:
                address_parts = re.match('(.*),\\s*(\\D{2,}\\s?\\D{2,}?\\s?\\D*)\\s(\\D{2})', wash['address'])
            if not address_parts:
                address_parts = re.match('(.*),\\s?(\\D*)', wash['address'])
            try:
            ...
            yield Feature(lat=float(wash['lat']), lon=float(wash['lng']), phone=phone, website='http://mistercarwash.com/locations/' + wash['name'].lower().replace(' ', '-'), ref=wash['loc_id'], opening_hours=self.store_hours(wash['loc_hours']), addr_full=address_parts[1], city=address_parts[2], state=state, postcode=zip_code, country=country)
    requires_proxy = True
    requires_proxy = True
    requires_proxy = True