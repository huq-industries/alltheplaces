...
class ChemistWarehouseAUSpider(scrapy.Spider):
    ...
    start_urls = ['https://www.chemistwarehouse.com.au/ams/webparts/Google_Map_SL_files/storelocator_data.ashx?searchedPoint=(0,%200)&TrafficSource=1&TrafficSourceState=0']
    requires_proxy = True
    ...
    def parse(self, response):
        for data in xmltodict.parse(response.text).get('markers').get('marker'):
            ...
            for day in DAYS_FULL:
                rule = data['@store' + day[:3].lower()].replace('.', ':').replace('00:00AM - 00:00AM', '12:00AM - 12:00AM')
