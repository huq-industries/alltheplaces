...
class ChicosSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for location in locations:
            ...
            properties = {'name': data['attributes']['name'], 'ref': data['id'], 'street_address': data['attributes']['address1'], 'city': data['attributes']['city'], 'state': data['attributes']['state'], 'postcode': data['attributes']['postalCode'], 'country': data['attributes']['countryCode'], 'website': 'https://stores.chicos.com/s/' + data['attributes']['slug'], 'lat': float(data['attributes']['latitude']), 'lon': float(data['attributes']['longitude'])}
