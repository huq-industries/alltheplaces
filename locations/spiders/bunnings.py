...
class BunningsSpider(scrapy.Spider):
    ...
    start_urls = ['https://api.prod.bunnings.com.au/v1/stores?latitude=-23.12&longitude=132.13&currentPage=0&fields=FULL&pageSize=10000&radius=9000000']
    ...
    def start_requests(self):
        ...
        yield scrapy.Request(f'https://authorisation.api.bunnings.com.au/connect/authorize?response_type=token&scope=chk:exec cm:access ecom:access chk:pub &client_id=budp_guest_user_au&redirect_uri=https://www.bunnings.com.au/static/guest.html&nonce={nonce}&acr_values=', self.parse_guest_login_page)
    ...
    def parse_guest_login_page(self, response):
        ...
        for url in self.start_urls:
            yield scrapy.Request(url, self.parse, headers={'Authorization': 'Bearer ' + self.auth_token, 'clientId': self.client_id, 'country': 'AU', 'locale': 'en_AU'})
    ...
    def parse(self, response):
        ...
        for location in response.json()['data']['stores']:
            ...
            for day in location['openingHours']['weekDayOpeningList']:
                if not day['closed'] and day.get('openingTime') and day['openingTime'].get('formattedHour') and day.get('closingTime') and day['closingTime'].get('formattedHour'):
                    oh.add_range(day=day['weekDay'], open_time=day['openingTime']['formattedHour'], close_time=day['closingTime']['formattedHour'], time_format='%I:%M %p')
