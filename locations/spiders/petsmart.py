...
class PetSmartSpider(scrapy.Spider):
    ...
    start_urls = ('https://www.petsmart.com/store-locator/all/', 'https://www.petsmart.ca/store-locator/all/')
    ...
    def parse_store(self, response):
        ...
        if 'petsmart.ca' in response.url:
            ...
            country = 'US'
        ref = re.search('-store(\\d+)', response.url).group(1)
        ...
        properties = {'name': response.xpath('//h1/text()').extract_first(), 'addr_full': response.xpath('normalize-space(//p[@class="store-page-details-address"])').extract_first(), 'lat': lat, 'lon': lon, 'phone': response.xpath('normalize-space(//p[@class="store-page-details-phone"])').extract_first(), 'country': country, 'ref': ref, 'website': response.url}
    ...
    def parse_hours(self, elements):
        ...
        for day, hours in store_hours.items():
            if 'CLOSED' in hours:
                continue
            opening_hours.add_range(day=day_mapping[day], open_time=convert_24hour(hours[0]), close_time=convert_24hour(hours[1]))
