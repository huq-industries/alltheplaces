...
class ChilisSpider(scrapy.Spider):
    ...
    def parse_hours(self, hours):
        ...
        for hour in hours:
            opening_hours.add_range(day=DAY_MAPPING[hour['dayOfWeek']], open_time=hour['opens'], close_time=hour['closes'])
    ...
    def parse_store(self, response):
        ...
        properties = {'street_address': html.unescape(data['address']['streetAddress']), 'phone': data['telephone'], 'city': data['address']['addressLocality'], 'state': data['address']['addressRegion'], 'postcode': data['address']['postalCode'], 'country': 'US', 'ref': data['branchCode'], 'website': response.url, 'lat': float(data['geo']['latitude']), 'lon': float(data['geo']['longitude']), 'name': html.unescape(data['name'])}
