...
class JourneysSpider(StructuredDataSpider):
    ...
    def parse(self, response, **kwargs):
        for state in response.xpath('//select[@name="StateOrProvince"]/option/@value').getall():
            if state:
                yield FormRequest(url=response.url, formdata={'StateOrProvince': state, 'Mode': 'search'}, callback=self.parse_store_list)
    ...
    def pre_process_data(self, ld_data, **kwargs):
        ...
        for rule in ld_data['address'].pop('openingHours', []):
            if (m := re.match('(\\w+): (\\d+:\\d+ [AP]M) - (\\d+:\\d+ [AP]M)', rule)):
                ld_data['openingHoursSpecification'].append({'dayOfWeek': m.group(1), 'opens': m.group(2), 'closes': m.group(3)})
