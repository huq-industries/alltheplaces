...
class napa(scrapy.Spider):
    ...
    def parse3rdlevel(self, response):
        if response.xpath('//script[@type="application/ld+json" and contains(text(), "streetAddress")]/text()').extract_first():
            data = json.loads(response.xpath('//script[@type="application/ld+json" and contains(text(), "streetAddress")]/text()').extract_first())
            try:
                state = data['address']['addressRegion']
            except:
                state = ''
            properties = {'ref': data['url'], 'name': data['name'], 'street_address': data['address']['streetAddress'], 'city': data['address']['addressLocality'], 'state': state, 'postcode': data['address']['postalCode'], 'country': data['address']['addressCountry'], 'phone': data.get('telephone'), 'lat': data['geo']['latitude'], 'lon': data['geo']['longitude']}
