...
def download_and_process_atp_data(
    run_data: dict, bucket_name: str, zip_storage_path: str, output_path: str, max_retries: int
) -> None:
    """
    ...
    try:
        ...
        zip_gcs_path = f"{zip_storage_path}/dt={today}/"
        ...
        zip_blob_name = os.path.join(zip_gcs_path, "atp_data.zip")
        zip_blob = bucket.blob(zip_blob_name)
        logging.info(f"GCS zip URL: {zip_blob_name}")

        upload_url = zip_blob.create_resumable_upload_session()

        headers = {"Content-Type": "application/zip"}
        # Save the streamed response content directly to a temporary ZIP file
        with open("/tmp/atp_data.zip", "wb") as f:
            for chunk in response.iter_content(chunk_size=4096):
                if chunk:  # Filter out keep-alive new chunks
                    f.write(chunk)

        # Upload the ZIP file to GCS
        with open("/tmp/atp_data.zip", "rb") as f:
            response_upload = requests.put(upload_url, data=f, headers=headers)
        if response_upload.status_code != 200:
            logging.error(f"Error uploading the ZIP file to GCS: {response_upload.content}")
            return
        logging.info(f"Uploaded ZIP file to {zip_blob_name}")

        # Process each GeoJSON file in the ZIP archive
        with zipfile.ZipFile("/tmp/atp_data.zip", "r") as zip_ref:
        ...
        if os.path.exists("/tmp/atp_data.zip"):
            os.remove("/tmp/atp_data.zip")
